%NEWUOA-Trans-Matlab-Version-1.0 
%Copyright: Pengcheng Xie 
%Connect: xpc@lsec.cc.ac.cn

function [NF, X] = newuob(Func, Gunc, N, NPT, X, RHOBEG, RHOEND, IPRINT, MAXFUN, FLAG)
    ...
    switch flag
      ...
      case 310
        if (NF > NFTEST)
          ...
          F = Func(N, X);
          if (NF >= NPT + 1)
            [G2, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(X, NF - 1, FLAG, EPS, EPS1, EPS2, RAND);
            F = F + G2;
            ...
            if (NF >= NPT + 2)
              ...
              [G1, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(XBEG, NF - 2, FLAG, EPS, EPS1, EPS2, RAND);
              [G2, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(XBEG, NF - 1, FLAG, EPS, EPS1, EPS2, RAND);
              FBEG = FBEG + (G2 - G1);
              for I = 1:N
                XTEMP(I) = XOPT(I) + XBASE(I);
              end
              [G1, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(XTEMP, NF - 2, FLAG, EPS, EPS1, EPS2, RAND);
              [G2, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(XTEMP, NF - 1, FLAG, EPS, EPS1, EPS2, RAND);
              FOPT = (G2 - G1) + FOPT;
            ...
            [G1, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(X, NPT, FLAG, EPS, EPS1, EPS2, RAND);
            F = F + G1;
          ...
          elseif (KNEW == -1)
            ...
            if (NF >= NPT + 1)
              [G2, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(XTEMP, NF, FLAG, EPS, EPS1, EPS2, RAND);
              [G1, FLAG, EPS, EPS1, EPS2, RAND, ~] = Gunc(XTEMP, NF - 1, FLAG, EPS, EPS1, EPS2, RAND);
